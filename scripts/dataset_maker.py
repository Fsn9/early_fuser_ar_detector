...
AREA_THRESHOLD = 800 # area in pixel coordinates to be considered a marker
IMG_HEIGHT = 0
IMG_WIDTH = 0
...
def make_dataset():
	...
	folders = [rospy.get_param("dataset_to_label_folder")]

	# Label entire dataset dir
	label_entire_dir = rospy.get_param("label_entire_dir")
	if label_entire_dir:
		folders = os.listdir(os.path.join(pkg_path, "datasets"))
		folders = list(filter(lambda folder_name : "bag" in folder_name, folders))
	
	for dataset_dir_path in folders:
		...
		for image_name in image2detection_data.keys():
			...
			cv2.putText(image, str(markerID),
				...
				0.5, (0, 255, 0), 2)
			min_x = min([topLeft[0], topRight[0], bottomLeft[0], bottomRight[0]])
			min_y = min([topLeft[1], topRight[1], bottomLeft[1], bottomRight[1]])
			max_x = max([topLeft[0], topRight[0], bottomLeft[0], bottomRight[0]])
			max_y = max([topLeft[1], topRight[1], bottomLeft[1], bottomRight[1]])
			cx, cy, w, h = cX / IMG_WIDTH, cY / IMG_HEIGHT, (max_x - min_x) / IMG_WIDTH, (max_y - min_y) / IMG_HEIGHT
			print(f'[INFO] annotation: cx:{cx} cy:{cy} w:{w} h:{h}\n')
			...
			if image_file in images_train:
				with open(os.path.join(train_dir, "labels", image_name + ".txt"), "w") as label_file: label_file.write(f"0 {cx} {cy} {w} {h}")
			elif image_file in images_test:
				with open(os.path.join(test_dir, "labels", image_name + ".txt"), "w") as label_file: label_file.write(f"0 {cx} {cy} {w} {h}")
			else:
				with open(os.path.join(valid_dir, "labels", image_name + ".txt"), "w") as label_file: label_file.write(f"0 {cx} {cy} {w} {h}")
